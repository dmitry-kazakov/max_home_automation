...
--        end if;
   ...
   begin
      if Recurse and then Widget.all in Gtk_Container_Record'Class then
         if Widget.all in Gtk_Notebook_Record'Class then
            declare
               Notebook : Gtk_Notebook_Record'Class renames
                          Gtk_Notebook_Record'Class (Widget.all);
               Page     : Gtk_Widget;
               Label    : Gtk_Widget;
            begin
               for Index in 1..Notebook.Get_N_Pages loop
                  declare
                     N : constant String := GInt'Image (-Index);
                  begin
                     Page := Notebook.Get_Nth_Page (Index - 1);
                     Put
                     (  File,
                        Page,
                        Recurse,
      ...
      begin
         -- Class  feature  is  broken  in latest Gtk. It is replaced by
         -- style classes unrelated to the widget  classes.  In  effect,
         -- only the widget name works. It is possible to attach a style
         -- class  to  a  particular  widget,  but  it is uselss for the
         -- purpose. The  base  classes  have  class  styles  which  are
         -- inherited.  E.g.  "GtkButton"  class  has  the  style  class
         -- "button". This can be used in  the  form  <class>#<name>  as
         -- before.
         if Class_Name = Get_Name (Widget) then
            -- The name  is likely not set,  do not use it,  that  won't
            -- work anyway
            Put_Line (File, Get_CSS_Name (Class));
         else
            Put_Line
            (  File,
               Get_CSS_Name (Class) & '#' & Get_Name (Widget)
            );
         end if;
