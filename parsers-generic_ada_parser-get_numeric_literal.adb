...
begin
   ...
   for Index in Mantissa..Mantissa + Fore - 1 loop
      case Line (Index) is
         when '.' | '_' | '0' =>
            Mantissa := Mantissa + 1;
            Fore     := Fore  - 1;
   ...
   if Real then
      for Index in reverse Mantissa + Fore + Aft - 1
                        .. Mantissa + Fore + 1 loop
         exit when Line (Index) /= '0';
         Aft := Aft - 1;
         Exponent := Exponent + 1;
      end loop;
   end if;
   if Fore + Aft = 0 then
      if Real then
         Argument.Value := new Real_Literal (1);
      else
         Malformed := Malformed or Exponent < 0;
         Argument.Value := new Integer_Literal (1);
      end if;
      declare
